...
import 'package:grizbee/data/datasources/mock_contacts.dart';
import 'package:grizbee/domain/entities/transaction.dart';
import 'package:grizbee/presentation/utils/random_date.dart';
...
Transaction generateRandomTransaction({required bool isToPay}) {
  ...
  double transactionValue = isToPay
      ? (random.nextDouble() * random.nextInt(100)).abs()
      : (random.nextDouble() * random.nextInt(100));
  if (transactionValue == 0) transactionValue = 123.45;
  if (!isToPay)
    transactionValue =
        random.nextBool() ? transactionValue : transactionValue * -1;
  ...
  if (isToPay) {
    ...
    while (transactionType == TransactionType.received) {
      transactionType = TransactionType
          .values[new Random().nextInt(TransactionType.values.length)];
    }
  }
...
List<Transaction> getTransactionsForTransaction(Transaction transaction) {
  ...
  for (final t in mockTransactions) {
    ...
    if (t.id != transaction.id && t.contact.id == transaction.contact.id)
      transactions.add(t);
  }
