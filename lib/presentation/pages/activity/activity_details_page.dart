...
import 'package:grizbee/presentation/pages/activity/activity_similar_page.dart';
import 'package:grizbee/presentation/utils/formatters.dart';
import 'package:grizbee/presentation/utils/modal_display.dart';
import 'package:grizbee/presentation/utils/translater.dart';
import 'package:grizbee/presentation/widgets/avatars/contact_avatar.dart';
import 'package:grizbee/presentation/widgets/sheets/bottom_sheets.dart';
...
class ActivityDetailsPageArguments {
  ...
  ActivityDetailsPageArguments(
      {required this.transaction, required this.showSimilarTransactions});
}
              ...
              child: Column(
                children: [
                  ...
                  _TransactionContact(
                      ...
                      showSimilar: args.showSimilarTransactions),
                ],
              ),
            ),
          ],
              ...
              children: [
                Text(
                  transaction.type != TransactionType.received
                      ? Translation.of(context).youHavePaid
                      : Translation.of(context).youHaveReceived,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(Translation.of(context).typeOfTransaction.toUpperCase(),
              style: TextStyle(fontSize: 20)),
          SizedBox(height: 10),
          Text(
              Translater.getTransactionTypeRawValue(context, transaction.type)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(Translation.of(context).detailsOfTransaction.toUpperCase(),
              style: TextStyle(fontSize: 20)),
          SizedBox(height: 10),
          _TransactionDetailsRow(
              title: Translation.of(context).amount,
          ...
          transaction.type != TransactionType.received
              ? Column(
                  children: [
                    SizedBox(height: 10),
                    _TransactionDetailsRow(
                        title: Translation.of(context).delivery,
                        amount: transaction.delivery,
                        currency: transaction.currency),
                    SizedBox(height: 10),
                    _TransactionDetailsRow(
                        title: Translation.of(context).tax,
                        amount: transaction.tax,
                        currency: transaction.currency),
                    SizedBox(height: 10),
                    _TransactionDetailsRow(
                        title: Translation.of(context).subtotal,
                        amount: transaction.subtotalAmount,
                        currency: transaction.currency),
                    Divider(),
                  ],
                )
          ...
          SizedBox(height: 10),
          _TransactionDetailsRow(
              title: Translation.of(context).fee,
              amount: transaction.fee,
              currency: transaction.currency),
          SizedBox(height: 20),
          _TransactionDetailsRow(
              title: Translation.of(context).total,
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text(
            '${AmountFormatter.format(context, amount.abs())} ${currency.rawValue}'
                .toUpperCase())
      ],
    );
  }
...
class _TransactionContact extends StatelessWidget {
  ...
  _launchEmail() async {
    final url = 'mailto:${transaction.contact.email}?subject=&body=';
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
              '${Translation.of(context).youAnd}  ${transaction.contact.name.toUpperCase()}',
              style: TextStyle(fontSize: 20)),
          SizedBox(height: 10),
          GestureDetector(
            child: _TransactionContactRow(
              ...
              title:
                  '${Translation.of(context).toContact} ${transaction.contact.name}',
            ),
            onTap: () => ModalSheet.show(
              context: context,
              content: TwoButtonsSheet(
                  title: transaction.contact.email, action: _launchEmail),
            ),
          ),
          Divider(),
          if (showSimilar)
              ...
              children: [
                ...
                GestureDetector(
                  child: _TransactionContactRow(
                      ...
                      title: Translation.of(context).similarTransactions),
                  onTap: () => Navigator.pushNamed(
                      context, RouteName.activity_similar,
                      arguments: ActivitySimilarPageArguments(
