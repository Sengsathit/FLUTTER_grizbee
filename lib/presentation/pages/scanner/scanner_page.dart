...
import 'package:grizbee/presentation/pages/scanner/transaction_payment_modal.dart';
import 'package:grizbee/presentation/utils/modal_display.dart';
import 'package:grizbee/presentation/widgets/dialogs/failure_dialog.dart';
import 'package:grizbee/presentation/widgets/loaders/loader_full_page.dart';
        ...
        BlocListener<ScannerBloc, ScannerState>(
          listener: (context, state) {
            switch (state.runtimeType) {
              case ScannerSuccess:
                context
                    .read<TransactionPaymentBloc>()
                    .add(GetTransactionToPayEvent());
                break;
          ...
          listener: (context, state) {
            switch (state.runtimeType) {
              ...
              case TransactionToPayLoaded:
                ...
                _showTransactionPayment(
                    context, (state as TransactionToPayLoaded).transaction);
                break;

              case TransactionPaymentProceeded:
                setState(() => loading = false);
                InfoSnackBar.showPaymentSnackBar(context,
                    (state as TransactionPaymentProceeded).transaction);
              ...
              case TransactionToPayFailure:
                ...
                InfoSnackBar.showErrorSnackBar(
                    context, (state as TransactionToPayFailure).message);
                break;

              case TransactionPaymentFailure:
                setState(() => loading = false);
                InfoSnackBar.showErrorSnackBar(
                    context, (state as TransactionPaymentFailure).message);
                break;
            }
          },
        ),
